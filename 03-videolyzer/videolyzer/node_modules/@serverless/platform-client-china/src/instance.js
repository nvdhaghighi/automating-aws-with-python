 ...
 */
const preRunSrcUpload = async (sdk, orgName, src) => {
  ...
  return [src, request.put(packageUrls.upload, body, config)]
}
...
const run = async (sdk, method = {}, instanceData = {}, credentials = {}, options = {}) => {
  ...
  let uploadTask
  if (
    (typeof instance.inputs.src === 'string' && instance.inputs.src) ||
    (typeof instance.inputs.src === 'object' &&
      (instance.inputs.src.src || instance.inputs.src.dist))
  ) {
    ...
    const [source, task] = await preRunSrcUpload(sdk, instance.orgName, instance.inputs.src)
